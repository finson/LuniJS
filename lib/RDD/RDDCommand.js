...
log.setLevel('DEBUG');
  ...
  let SYSEX = {
      'DEVICE_QUERY' : 0x5E,
      'DEVICE_RESPONSE' :0x5F
    };

  /**
   * PIN_MODE_DEVICE is not defined by Firmata, so I have used an existing
   * code that seems to fit.
   */
  let PIN_MODE_DEVICE = 0x10;  // The UNKOWN [sic] pin mode
  ...
  //--------------------
    ...
    class SemVer {
      ...
      constructor(buf) {
        ...
        log.trace(`offset: ${offset}, count: ${byteIndex-offset}`);
...
module.exports = {SYSEX, ACTION, DAF, MO, CDR, PIN_MODE_DEVICE,
    SemVer,
