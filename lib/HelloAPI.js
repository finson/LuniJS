 ...
 * defined in AbstractAPI, the superclass.
 */
class HelloAPI extends RDDAPI.AbstractAPI {
  ...
  getContinuousGreeting(handle){
    this.proxy.read(handle, RDDCmd.DAF.MILLI_RUN, RDDCmd.CDR.Stream, 256, (response) => {
      log.trace(`proxy.read(${response.handle}) callback invoked for API getContinuousGreeting().`);
      if (response.status >= 0) {
        this.emit("read-continuous",{status : RDDErr.STATUS("ESUCCESS"), handle : response.handle,
