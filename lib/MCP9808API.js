...
class MCP9808API extends API.DeviceAPI {
  ...
  setShutdown(handle,turnItOff) {
    this.proxy.read(handle, 0, REG.CONFIG, 2, (rA) => {
      if (rA.status >= 0) {
        if (turnItOff) {
          rA.datablock[1] |= MASK.SHUTDOWN;
        } else {
          rA.datablock[1] &= ~MASK.SHUTDOWN;
        }
        this.proxy.write(rA.handle,RDD.DAF.NONE,REG.CONFIG,2,rA.datablock,(rB) => {
          if (rB.status >= 0) {
            this.emit("write",{status : RDD.SC.ESUCCESS.val, handle : rB.handle});
          } else {
            this.emit("error",{status : rB.status, handle : rB.handle});
          }
        });
      } else {
        log.error(`Error value from read(config) is ${rA.status}`);
      }
    });
  }
