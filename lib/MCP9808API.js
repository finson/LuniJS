 ...
 */
  constructor(opts) {
    ...
    cbSetHighPower.push((response) => {
      if (response.status >= 0) {
        ...
        this.write(response.handle,RDDCMD.DAF.NONE,REG.CONFIG,2,response.datablock,cbSetHighPower[1]);
      } else {
    ...
    cbSetLowPower.push((response) => {
      if (response.status >= 0) {
        response.datablock[1] |= SHUTDOWN;
        this.write(response.handle,RDDCMD.DAF.NONE,REG.CONFIG,2,response.datablock,cbSetLowPower[1]);
      } else {
        log.error(`Error value from read(config) is ${response.status}`);
      }
    });
  ...
  readTempC(handle, callback) {
    ...
    this.read(handle, 0, RDDCMD.CDR.Stream, 256, cbReadTempC[0]);
  }
  ...
  beginContinuousReadTempC(handle, callback) {
    ...
    this.read(handle, RDDCMD.DAF.MILLI_RUN, RDDCMD.CDR.Stream, 256, cbBeginContinuousReadTempC[0]);
  }
