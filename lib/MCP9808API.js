...
const RDDAPI = require("../lib/DeviceAPI");
 ...
 */
class MCP9808API extends RDDAPI.DeviceAPI {
  ...
  setShutdown(handle,turnItOff) {
    this.read(handle, 0, REG.CONFIG, 2, (response) => {
      if (response.status >= 0) {
        if (turnItOff) {
        ...
        this.write(response.handle,RDD.DAF.NONE,REG.CONFIG,2,response.datablock,(response) => {
          if (response.status >= 0) {
