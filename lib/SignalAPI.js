 ...
 */
  setChannelList(handle,channelDescriptors) {
    this.proxy.write(handle,RDD.DAF.NONE,RDD.CDR.Configure,channelDescriptors.length,channelDescriptors,(response) => {
    ...
    this.proxy.read(handle, RDD.DAF.NONE, REG.CHANNEL_VALUES, 256, (response) => {
      ...
      if (response.status >= 0) {
        ...
        let byteIndex = 1;
          ...
          channelValues[0] = response.datablock.readInt16LE(1);
          channelValues[1] = response.datablock.readInt16LE(3);
          channelValues[2] = response.datablock.readInt16LE(5);
    ...
    this.proxy.read(handle, RDD.DAF.MILLI_RUN, REG.CHANNEL_VALUES, 256, (response) => {
      ...
      if (response.status >= 0) {
        this.emit("read-continuous",{status : RDD.SC.ESUCCESS.val, handle : response.handle,
          flags : response.flags, data : response.datablock});
      } else {
        this.emit("error",{status : response.status, handle : response.handle});
      }
    });
