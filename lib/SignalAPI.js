 ...
 */
  ...
  setChannelList(handle, direction, channelDescriptors) {
    let pb = new Buffer(2+channelDescriptors.length);
    let byteIndex = 0;
    pb[byteIndex++] = direction;
    pb[byteIndex++] = channelDescriptors.length;
    for (let c = 0; c < channelDescriptors.length; c++) {
      if (!channelDescriptors[c].hasOwnProperty("digitalPin")) {
        if (channelDescriptors[c].hasOwnProperty("analogPin")) {
          channelDescriptors[c].digitalPin = RDD.board.analogPins[acd.analogPin];
        } else {
          this.emit("error", {status: RDD.SC.EBADR, handle: handle});
        }
      }
      pb[byteIndex++] = channelDescriptors[c].op;
      pb[byteIndex++] = channelDescriptors[c].config;
      pb[byteIndex++] = channelDescriptors[c].digitalPin;
    }
