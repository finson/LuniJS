 ...
 */
  ...
  setIntervals(handle, micros, millis) {
    this.proxy.read(handle, 0, RDD.CDR.Intervals, 8, (rA) => {
      log.trace(`proxy.read(${rA.handle}) callback invoked for API setIntervals().`);
      if (rA.status >= 0) {
        if (typeof micros === "number") {
          rA.datablock.writeUInt32LE(micros,0);
        }
        if (typeof millis === "number") {
          rA.datablock.writeUInt32LE(millis,4);
        }
        this.proxy.write(rA.handle,RDD.DAF.NONE,RDD.CDR.Intervals,8,rA.datablock,(rB) => {
          log.trace(`proxy.write(${rB.handle}) callback invoked for API setIntervals().`);
          if (rB.status >= 0) {
            let result = {
              status : RDD.SC.ESUCCESS.val,
              handle : rB.handle,
